// -*- coding: utf-8 -*-
// 插件版 AirDrop 前端上传状态机 jsdom 验证
// 用 jsdom 自动执行真实上传脚本，验证：小文件整传成功 / 暂停 / 继续恢复 / 取消 / 卡住检测。
// 方法：读 package/templates/index.html，替换 Jinja；runScripts:'dangerously' + beforeParse 注入
//       mock XHR/fetch/confirm/时钟，让脚本在真实 DOMContentLoaded 时序下自动绑定；触发 UI 后断言 DOM 状态。
// 与独立版差异：URL 前缀 /api/airdrop/。
// 运行：node tools/verify_chunk_ui_plugin_jsdom.js
const fs = require('fs');
const path = require('path');
const { JSDOM, VirtualConsole } = require('jsdom');

const TPL = path.join(__dirname, '..', 'package', 'templates', 'index.html');
let html = fs.readFileSync(TPL, 'utf8');
html = html.replace(/\{\{\s*chunk_size_mb\s*\|\s*default\(\s*8\s*\)\s*\}\}/g, '8');
html = html.replace(/\{\{\s*addresses\s*\|\s*tojson\s*\}\}/g, '[]');
html = html.replace(/\{\{ theme_effective \}\}/g, 'auto');
    ...
    beforeParse(window) {
      ...
      window.XMLHttpRequest = class MockXHR {
        ...
        send() {
          ...
          if (this.url === '/api/airdrop/upload') payload = { status: 'success' };
      ...
      window.fetch = async (url) => {
        if (url.includes('/api/airdrop/upload/status')) return { json: async () => ({ status: 'ok', received: [], total_chunks: 1 }) };
        if (url.includes('/api/airdrop/upload/complete')) return { json: async () => ({ status: 'success', msg: '上传成功: t.bin' }) };
        if (url.includes('/api/airdrop/upload/abort')) return { json: async () => ({ status: 'ok' }) };
...
(async () => {
  ...
  env.selectFile('small.txt', 'hello plugin chunk');
