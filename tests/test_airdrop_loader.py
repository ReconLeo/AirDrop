...
本文件已移交 AirDrop 子项目维护（2026-09-06，原 FlaskToolkit/tests/test_airdrop_loader.py）。
运行需要 FlaskToolkit 框架代码：设置环境变量 FLASKTOOLKIT_ROOT 指向框架仓库根（默认取本文件
上级目录，放回框架 tests/ 下运行时无需设置）。
...
运行：FLASKTOOLKIT_ROOT=<FlaskToolkit仓库根> python tests/test_airdrop_loader.py
"""
import os
import shutil
import sys
...
# FlaskToolkit 仓库根：优先环境变量 FLASKTOOLKIT_ROOT（本文件已移交 AirDrop 子项目 tests/，
# 需指向框架仓库以复用 app / core / plugins 骨架）；默认取文件上级目录（放回框架 tests/ 下不变）。
_PROJECT_ROOT = os.environ.get('FLASKTOOLKIT_ROOT', os.path.dirname(_TESTS_DIR))
sys.path.insert(0, _PROJECT_ROOT)

# AirDrop 插件文件（plugins/airdrop.py 及模板）属独立子项目，不入 FlaskToolkit 仓库
# （CI clone 后不存在）。插件落地到框架 plugins/ 后运行本回归，缺失时跳过。
if not os.path.exists(os.path.join(_PROJECT_ROOT, 'plugins', 'airdrop.py')):
    print(f"SKIP: {os.path.join(_PROJECT_ROOT, 'plugins', 'airdrop.py')} 不存在"
          f"（AirDrop 插件需落地到框架后运行；若从子项目运行请设置 FLASKTOOLKIT_ROOT 指向 FlaskToolkit 仓库根）")
    sys.exit(0)
