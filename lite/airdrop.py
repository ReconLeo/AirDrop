...
class AirDropPlugin(BasePlugin):
    ...
    def get_lan_addresses(self):
        """获取所有非 localhost 的局域网 IP。
        优先复用框架 core.network（主框架 v4.11+）；Lite 4.2.2 无 core.network，回退纯 socket 枚举。"""
        try:
            from core.network import get_lan_addresses as _get_lan_addresses
            return _get_lan_addresses()
        except ImportError:
            self.logger.info("框架无 core.network（Lite），回退纯 socket 枚举局域网地址")
            return self._fallback_lan_addresses()
        except Exception as e:
            self.logger.error(f"获取网络地址失败: {e}")
            return []

    def _fallback_lan_addresses(self):
        """Lite 无 core.network 时的回退：纯 socket 枚举本机局域网 IP（不引入 subprocess）。"""
    ...
    def _sanitize_filename(self, filename):
        """Lite 4.2.2 子集版文件名净化（主框架用 BasePlugin.sanitize_filename，Lite 无）。"""
        filename = re.sub(r'[\\/:*?"<>|\n\r\t]', '', filename)
        filename = filename.replace('..', '').lstrip('.')
        if not filename.strip():
            filename = f"未命名文件_{int(time.time())}"
        return filename

    def _save_uploads_sync(self, key='files', dest_dir=None):
        """Lite 4.2.2 子集版同步上传助手：净化 + 重名去重 + 单文件大小预检 + 落盘（无存储配额）。
        返回结构与主框架 save_uploads 一致（部分成功语义）。"""
        files = request.files.getlist(key)
        if not files:
            raise ValueError('未选择文件')
        dest = os.path.abspath(dest_dir or self.upload_folder)
        ...
        for file in files:
            ...
            oversize = self.check_upload_limit(file, None)
            if oversize:
                results.append({'status': 'rejected', 'original_name': original,
                                'saved_name': original, 'path': None, 'size_bytes': 0,
                                'reason': 'size_exceeded', 'limit_mb': None, 'remaining_mb': None})
                continue
            saved_name = self._sanitize_filename(original)
            ...
            while os.path.exists(save_path):
                ...
                n += 1
            file.save(save_path)
            results.append({'status': 'saved', 'original_name': original, 'saved_name': saved_name,
    ...
    def upload_files(self):
        # Lite 子集版同步上传助手（_save_uploads_sync）：净化 + 重名去重 + 单文件大小预检 + 落盘（无存储配额）
        try:
            results = self._save_uploads_sync('files', dest_dir=self.upload_folder)
